...
\begin{figure}[tb]
    \centering
    \includegraphics[width=.6\textwidth]{binary-space-partition.jpg}
...
\lstinputlisting[language=Java,alsolanguage=Java,]{tight_staircase_conditional.java}
...
An exception is Golang where a new variable may be declared and remain visible
thereafter \emph{within} the cascading conditionals for example:
\lstinputlisting[basicstyle=\footnotesize]{intermediate_var.go}

\begin{figure}[h]
  \centering
  \begin{lstlisting}[caption={This example demonstrate organizing cascading if statements the right way.}]
        Original
...
\begin{clisting}
    \begin{minipage}[t]{.4\textwidth}
        ...
        {modified_tight_staircase_conditional.java}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{.4\textwidth}
        ...
        {refactor_staircase_conditional.java}
    \end{minipage}
\end{clisting}
...
\begin{clisting}[htb]
    ...
    lastline=12]
    {nested_conditional_2_cascading_conditional.py}
\end{clisting}
...
\lstinputlisting[firstline=15,lastline=24]{nested_conditional_2_cascading_conditional.py}
...
Finally, we get:
\lstinputlisting[firstline=28]{nested_conditional_2_cascading_conditional.py}
