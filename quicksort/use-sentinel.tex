...
But a large part of the reason is to remove clutter from the code.
...
So by use of sentinels on the Hoare's quicksort algorithm, traditional nested loop implementation
or the single-loop implementation ~\autoref{lst:singleLoopHoarePart} alike,
...
Applied to the latter, we arrive at a new level of simplicity for the
implementations of Hoare's quicksort (as shown in ~\autoref{lst:hoare-partition-sole-loop-sentinel}).
