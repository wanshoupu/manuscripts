...
As can be seen, the program acquires one assertion undergoing through each test in a staircase
conditional, as if collecting a stamp in the course of a scavenger hunt.
This is what I meant by `cumulative effect' and
...
On the other, the cumulative effect of staircase conditionals makes it more challenging to
keep one's mind synchronized with one's code from time to time.
This put a demanding requirement on quality of developer in arranging
conditional statements in a staircase.


\subsection{How to Arrange Conditionals in Staircase}
\label{subsec:orderingStaircaseConditional}

%TODO

Coming up with that ordering takes some deliberation.
To produce correct, optimal and succinct code, I almost always advise developers to use due
diligence.
Many times it is necessary to arrange and rearrange the conditional blocks back and forth.
After each arrangement or rearrangement, make sure that one
double-checks the assumptions for each line of code is asserted by earlier tests.
...
First, coverage.
Secondly, the non-overlapping
Thirdly, the ordering
