...
\begin{quote}
    ...
    \begin{enumerate}
        \item if \code{flag = true}, pop from the minimum end of the \code{deque};
        \item Otherwise, pop from the maximum end.
    \end{enumerate}
    One can assume the \code{deque} is non-empty.''
\end{quote}
...
The second solution is based on the first but is a refactored version.
...
If you haven't made it your habit to strive for the `great' code, do not panic.
The rest of this chapter is dedicated to show you how.

``As long as it works, who cares if the logic is the simplest?'' some may insist.
'Barely working' solution often contains code that is buggy, brittle, verbose and/or smelly.
...
Especially, you will learn how to routinely arrive at the one-liner solution out
of the `first working' one in the above challenge problem.
...
\subsection{Unnest nested conditionals}\label{subsec:unnestNestedConditionals}

Above, through case studies, we formed step-by-step formula on how to produce cascading
conditionals from scratch.
What if one works on existing code that contains suboptimal conditionals?
For example condition the following code snippet where
\code{lookup} is a dictionary (or a \code{HashMap}) with integer key and integer array value.
Additionally, the integer arrays satisfy \code{heap} properties.
\begin{clisting}
    \lstinputlisting[label={lst:conditional-push-up-example},caption={},lastline=12]
    {if-else/nested_conditional_2_cascading_conditional.py}
\end{clisting}
How do we go about to refactor nested conditionals to cascading conditionals?
In this subsection, you are going to learn a technique dubbed as `construct push-up'.
The key is to identify and promote a condition in the nested conditional so that it joins the
upper level conditional to form a cascading conditional.

Taking the ~\autoref{lst:conditional-push-up-example} as an example.
%TODO
\clearpage
\lstinputlisting[firstline=14]{if-else/nested_conditional_2_cascading_conditional.py}

Most importantly,
Through the above example, I want to urge the reader to form a `code-sense', an instinct, a habit
of thinking.
When coming to a similar situation, if one know what lean code would look like, one is more
likely motivated to achieve such level of neatness.


Of course, cascading conditional may not be the best solution still.
As you will learn later, the complexity of conditionals can usually be designed away with
...
We will talk about this in more details in ~\autoref{ch:ood}.
