...
Together, we witnessed the refinement process.
...
With the specific problem in hand, some the techniques we explained in earlier chapters may
resonate with the problem with a crispy `Ding' sound!
That's amazing achievement.

We are going to advance into the second half of the project---formatting the in-memory data
structure representatino of the JSON data into pretty text---and we are going to experience some
miraculous refactor to simplify what looks incurable code.

\section{Json formatter}\label{sec:jsonFormatter}

%TODO
An initial implementation ~\autoref{lst:JsonFormatter.firstVersion}

It takes an in-memory object representation of JSON data, print it into well-formatted string in
