...
is still not straightforward and obstructs readability.
With the use of sentinels, we can get rid of that clutter.
But the end result is shown in the below listing.
\lstinputlisting[label={lst:hoare-partition-sole-loop-sentinel},
caption={Hoare's partition function with use of sentinel}]
{quicksort_partition_single_loop_sentinel.c}
This partition function would not work with ~\autoref{lst:QuickSortOutline} as is.
The modification necessary is to add another base case when there are fewer than $3$ elements in
the array to be sorted.
The more detailed discussion can be found in ~\autoref{sec:quick-sort-sentinel}.
