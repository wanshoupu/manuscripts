...
As you may notice there is a helper function \code{def whitespace(indent)} where \code{indent} is
...
Firstly, let us take a look at the function \code{dictToPrettyString}.
...
Take for example the function \code{arrayToPrettyString}, we can refactor it in such a way.
...
an element.
The refactored code, again for \code{arrayToPrettyString} is shown in
...
\code{dictToPrettyString} in the same way.
...
The new implementation does not use a function such as \code{def whitespace(indent)} to do the
indentation.
