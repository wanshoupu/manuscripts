...
The complicxity of the problem is doubled, tripled the moment we decide to send one pointer for each
end of the array to meet in the middle.
...
(see blow ~\autoref{lst:nestedLoopOnePointer}).
...
As expected, the simplicity of this program comes with performance penalty
for certain cases,
...
``Can one implement quicksort with the simplicity but not the
