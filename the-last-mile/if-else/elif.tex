...
Occasionally, we may be able to get away with a sequence of
\code{if} statements (instead of `\code{else if}' or \code{elif})
with \code{return}, \code{continue}, or \code{break} statements,
when the construct is the last statement to be executed in a function or a loop construct.
...
\begin{itemize}
    ...
    \item First key on the left subtree of root node, second key on the right subtree
    (\textit{a.k.a} FLSR)
    \item First key on the right subtree of root node, second key on the left subtree
    (\textit{a.k.a} FRSL)
    \item Two keys are on different subtrees of the root node.
    (\textit{a.k.a} STRADDLING-KEYS)
...
Our worksheet is presented in ~\autoref{tbl:case-walk-thru} and this will provide some insights
for next step.
...
To sum up, the remaining `true' distinct cases needing special handling are
\begin{itemize}
    \item Null BST
    \item Non-existing keys
    \item `LEFT-SKEWED'
