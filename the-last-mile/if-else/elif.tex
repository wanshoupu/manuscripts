...
\section{Conditional Staircases}\label{sec:conditional-staircase}
...
It begins with a \code{if}-conditional, followed by zero, one, or more of
\code{else if} conditional blocks (\code{elif} for \python),
and optionally concludes with an \code{else} block.
This train of conditional statements is an important pattern for coding and it is present
everywhere even in programing primer books.
However it was not formally discussed as a conditional construct.
Little is mentioned regarding this patterns in popular programming references.
Developers experience this little programming gem by stumbling upon it haphazardly.
To promote best practices around it, I am going to cover this here in some detail.


For the sake of argument, we refer to such a construct as `conditional staircase', as it
resembles to a stairway with a set of steps on top of one another.
...
So some of inexperienced developers end up with nested \code{if..else} statements such as the
