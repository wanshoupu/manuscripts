...
Loop pacing is another tuning dial when it comes to optimize a loop construct.
...
nonlinear stepping.
When using C-style \code{for}-loop for linear stepping,
do try to fit loop-state updating statements in the afterthought.


For simple use cases, the usual choices can be anywhere between deterministic linear stepping like
\code{for(int i=0; i<list.size();i++)}, or nonlinear, conditional stepping.
For complex cases, the loop updating model can be like that of an automaton, a state machine.
For an extreme case of nonlinear stepping, consider the `Happy number'
...
\end{figure}
One can get a sense of how `nonlinear' it can be for a loop pacing through this case.

Tuning how a loop is implemented my involve changing where the state-updating statements is
placed---in afterthought of a \code{for}-loop or in a conditional block within the loop body.
This can be particularly helpful when one performs `loop rotations' or compacting nested loops
(see ~\autoref{ch:loop-thinning}).
