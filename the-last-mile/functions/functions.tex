...
The code in ~\autoref{lst:ValidNumberNeedRefactor} illustrates the idea of what are modular functions and
how they are created to be reused.

\begin{clisting}
    \lstinputlisting[label={lst:ValidNumberNeedRefactor},firstline=3,language=Java,escapechar=~,
    caption={Numerical string validation.}]{functions/ValidNumberToBeRefactored.java}
\end{clisting}

\begin{clisting}
    \lstinputlisting[label={lst:ValidNumberRefactoredMain},firstline=3,language=Java,escapechar=~,
    caption={Numerical string validation.}]
