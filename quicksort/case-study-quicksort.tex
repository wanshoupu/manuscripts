...
Originally invented along with the quicksort algorithm, Hoare's partition scheme avoids the
drawback of Lomuto's with symmetric traversal: one pointer
from each end traversing toward each other\cite{Hoare1961,Hoare1962,Sedgewick1978}.

While a seemingly minor change in array traversal, the implementation complexity it brings
is immense.
So much so that for a long time Hoare's partition scheme is enigmatic\cite{Bentley-Beautiful-code}.
It is hard for one to design a test case to hit specific edge case.
Also each attempt of implementation one is stuck at different places, came to a differing
solution every once a while.

But one thing in common: most of the solutions contain three loops: one outer loop with two
inner loops should-by-shoulder.
That has been so commonplace that many think it is the stereotype of quicksort---it is inevitable.
Or is it?
...
\begin{enumerate}
    ...
    developers.
    \item Pointer inclusiveness and/or exclusiveness when passing the array with two pointers,
    the \code{start} and the \code{end}.
    It turns out that both conventions can work with no excessive inconvenience.
\end{enumerate}
Any of the items in this list can easily take a good chunk of implementation and/or debugging time.
Just to debug ~\autoref{itm:quicksort-boundary-condition} for this article, for example, it took a
good $2$ hours out of me.
...
would the returned pointer change to?
\code{s}, \code{s+1}, \code{e}, or \code{e-1}?
Accompanying these changes, the recursive calls in ~\autoref{lst:nestedLoopHoareMethod} line $4$-$5$
also need changes.
Should these lines be changed to
~\autoref{lst:return-val-left} or ~\autoref{lst:return-val-right}?
\begin{minipage}[t]{.4\textwidth}
    \begin{lstlisting}[caption={A},label={lst:return-val-left},language=C]
        qsort(s, p - 1)
        qsort(p + 1, e)
    \end{lstlisting}
\end{minipage}\hfill
\begin{minipage}[t]{.4\textwidth}
    \begin{lstlisting}[caption={B},label={lst:return-val-right},language=C]
        qsort(s, p + 1);
        qsort(p + 2, e);
    \end{lstlisting}
\end{minipage}
...
These considerations illustrate how the choices are highly dependent on one another.
Apart from these complexity, the traditionally, the implementations inevitably involves three
nested loops as shown in ~\autoref{lst:nestedLoopHoareMethod}.
