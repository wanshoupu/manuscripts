...
Here we refer to such a train of conditional statements as `\emph{staircase conditional}',
for its resemblance to a flight of stairs built on one another.
Sometimes we refer to `\emph{staircase conditional}' simply as `\emph{staircase}', or its shorthand
\emph{SC}.
Below are some examples of staircase conditionals
\begin{clisting}[h]
    \begin{minipage}[t]{.4\textwidth}
        \lstinputlisting[label={lst:staircaseCondElif},
...
\begin{clisting}[tbh]
    ...
    ~\autoref{lst:staircaseCondElif} (in \java)},
...
One of the key advantage of using staircase conditional is to reduce the number of cases one has
to check.
This reduction in number of tests would be made clear if one expands the staircase conditional
into non-cascading conditionals.
For example, in ~\autoref{lst:cumulative-tests-staircase} the number of tests checked is $3$
...
The cumulative effect of staircase conditionals put a strict ordering among
...
when programming staircase conditionals.
...
when debugging staircase conditional in ~\autoref{lst:cumulative-tests-staircase}.
...
The BSP scheme for ~\autoref{lst:staircaseCondElif} is plotted in ~\autoref{fig:bsp}.
