...
\chapter{Spiral Matrices}\label{ch:spiralMatrix}

The spiral matrix problems refers to visiting a $2$D matrix in clockwise spiral order
starting from $M[0][0]$, outer layer first as shown in ~\autoref{fig:spiral-matrix-loop}.
...
\begin{figure}[H]
    \begin{center}
        \includegraphics[width=.8\textwidth]{case-study/spiral-matrix/spiral-matrix.jpg}
...
a general algorithm for the family of spiral matrix problems may be
\begin{quote}
...
~\autoref{lst:VerboseSpiralMatrix.py} list an earlier implementation I myself managed to complete.
...
Admittedly, this is not the very first version of working solution I ever came up.
...
As warned in ~\autoref{complication-4}, be sure to handle the special base case properly
...
\lstinputlisting[label{lst:pseudo-recursive-spiral-order},lastline=35,
...
the four sections.
Pick and try one way yourself and see which way may be more conveniently coded.
...
Watch out for the \python language feature when you use slicing with a negative step all the way
to the zeroth element.
The only working syntax for that is \code{arr[i::-1]} with \code{i} between $(0, len(arr))$.
...
\lstinputlisting[label={lst:spiral_order_recursive},
