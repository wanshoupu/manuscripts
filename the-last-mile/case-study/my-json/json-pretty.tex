...
The intended `public' function is
\begin{lstlisting}
def toPrettyString(obj, indent=0)
\end{lstlisting}
and and other private functions recursively call one another.

List comprehension is a supported programming language feature in \python.
Maybe we can make the above implementation more compact by using it.
...
Still the implementation smells.
There is a lack of symmetry in the code,
and because of it, everything seems so `manual'.
Combination of string segments such as \code{",\n"} seems arbitrary and redundant to \code{"\n"}.
It seems we really twisted our brain to come up with overly complicated logic in order
to achieve the insertion of field separator \code{,} for ``all-but-last-line''.
(The part of code where we enumerate with indexes along with item for dictionary and list.)
Extra function parameter \code{indent} in function definition with default that's never used
other than within the recursive call in function
\begin{lstlisting}
def toPrettyString(obj, indent=0)
\end{lstlisting}
This parameter is there only because it is needed by the other recursive methods.
This is the so-called `remnant function parameter'.
See \autoref{sec:remnantFunctionParameter} for more info.
...
\[\mbox{line} = \mbox{line content} +\mbox{newline}\]
to a rotated form
\[\mbox{line} = \mbox{newline} \mbox{line content}.\]
