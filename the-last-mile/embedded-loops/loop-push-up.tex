...
This is a classical example that I borrowed from the book `Programming Pearls' for demonstration of
the simplicity programming loops can be\cite{Bentley:1986:PP}.

Before reading this book, I implemented several versions of the quick sort algorithm.
Interested reader may refer to that classical book by Bentley for more detail.

For example, my first implementation of a quick sort algorithm was in

\begin{clisting}
    \lstinputlisting[label={lst:QuickSortOutline},
    caption={Quicksort implementation outline with the partition function `\code{part}' to be
    implemented.}]
...
\begin{clisting}
    \lstinputlisting[label={lst:nestedLoopTwoPointer},
    caption={Implementation of partition function for Quicksort using two pointers.}]
    {embedded-loops/quicksort_partition_two_pointer.c}
\end{clisting}

\begin{clisting}
    \lstinputlisting[label={lst:nestedLoopOnePointer},
    caption={Implementation of partition function for Quicksort using one pointer.}]
    {embedded-loops/quicksort_partition_one_pointer.c}
\end{clisting}
