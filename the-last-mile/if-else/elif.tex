...
Here we refer to such a train of conditional statements as `\emph{cascading conditional construct}',
for its resemblance to a flight of stairs built on one another.
Sometimes `\emph{cascading conditional}', `\emph{cascade}', or the shorthand \emph{CCC} will be
...
Cascading conditional is offered as an alternative to nested conditionals
...
As ~\autoref{lst:simpleBranching} shows, cascading
...
We have elaborated a lot about cascade above.
...
Once again, you see how the program makes its way up a `staircase', one step at a time.
Also you see that the cascading conditional construct we used here conveniently permits the creation
...
\begin{clisting}
    ...
    {if-else/lca.java}
\end{clisting}
Earlier we pointed out the strict ordering in cascading conditional.
To illustrate how the order of tests affects the construct of cascading conditional,
...
However, with a properly designed cascading conditional as we are going to show next,
...
compact, sequential statements, that essentually of a `cascading conditional'.
\begin{clisting}[ht]
