...
Ever since software industry became an industry, the demand growth of qualified professionals far
outgrows that of supply.
But qualified professionals have remained as a rare commondity.
As a result, many famous projects came with a great design but only implemented with mediocrity.
To convince yourself, take peek at repositories at \url{github.com}, search the famous open source
code that we rely on such as Apache Common Libraries, Hadoop, Spark, \textit{etc.}.
It should not take long to identify the `smells' as listed in Martin Fowler's ``Refactoring'' book.

Rather, the gap is larger each year in the past 20 years.

In Forrester's 2018 report, it cites that software engineering talent shortage is quality,
not just quantity.
