...
This solution is also reminiscent to concurrent programming with two threads, one for deposit
and the other for withdrawal.
And indeed one may adapt the code to a multi-threaded program similar to the
well-known `producer-consumer' problem.
For completeness, some implementations start with an open set---a set of indexes that are not
sufficient to cover the character deficit.
But I found that starting with a closed set---a contained window---and then trying
to shorten it is more manageable.
This is similar to fighter aircraft chasing target---Lock and follow is the primary strategy,
if at all possible.
...
\end{itemize}
As a result these issues, our first version of solution is very delicate and buggy.
...
First of all, we can solve the duplicate code resulting from the `loop and a half' problem.
...
However, there are still something that I believe can be improved in
\autoref{lst:MinWindowSubstringSentinel}.
Firstly, the increment of the left and right boundaries both checks on the same condition---if
the \code{charAt} is relevant to the target string.
I consider this duplicate code.
Also, because of this check, the \code{for}-loop was made fragmented and lengthy.
Note the \code{break} statement in the middle?
Note the the span of the variable \code{c}?
Note the two occurrences of \code{++left}?
Note the two nullity checks for \code{count}?
All these convey the signal that there are still room for improvement.
