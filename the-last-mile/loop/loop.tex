...
Many times, duplication may not be so obvious as `copy-n-paste'.
Two code blocks may contain `soft duplication'---hidden, `quavery', and partially duplicate
code, for example,
\lstinputlisting[]{loop/soft_dup.py}
`Soft duplication' is tougher to detect and remove.
It is only manifest and accessible after a round of pre-refactors.
In that case, one should perform due pre-refactors before applying loop rotation.
...
In light of the above discussion of `loop rotations', let us now examine the impact of
initialization on a loop.

Initialization is a preclude, a bridge to the loop body,
It is also a preparatory primer coating that prepares the substrate for a loop.
