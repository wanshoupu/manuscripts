...
\end{lstlisting}
The loop conditions here are awkwardly complicated and pose tremendous difficulty for us to follow
the `nested loop thinning` refactoring procedure outlined in ~\autoref{sec:loop-thinning-technique}.
On the one hand, the pre-incremental statements and ruthlessly ratchet
the value `\code{++s}' (`\code{--e}') up(down) regardless to what the loop condition evaluates to;
...
We swapped ~\code{--e} up in front of the first inner loop
as verifiably the correctness is not affected.
The solution may be more messy than before.
But it is necessary and beneficial for the `loop rotation' procedure next
(refer to ~\autoref{sec:LoopDesignConsideration}) which, in turn, moves the \emph{pre-inner-loop
statements} out of the way for a smooth loop rotation.
After this step, the code becomes:
\lstinputlisting[language=C]{quicksort_partition_sym_rotated_inner_loop.c}
Again, we did extra things by tucking the \emph{post-inner-loop statements} to a conditional block.
Now we are ready with the third and most exciting step---converting inner loops to conditionals.
Note that we need to covert the second loop to an `\code{else if}' because the first inner loop is
converted to an `\code{if}' clause and the second is executed only the first is exhausted.
