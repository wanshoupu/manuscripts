...
Functions in test code may be lengthier than its production counterpart as test cases may be
combinatorial and that may be reflected in names.
...
Secondly, I have something to say about the cardinality of argument list.
My only advice is `Beat all the odds to keep it no more than 2'.
Sometimes you may have an urge to create a function with $>2$ arguments.
Please refrain from it and refer to \autoref{tbl:arg-shortening-tech} for techniques that may
...
Finally, a word on function return values.
...
Therefore, to solve a fairly complicated problem, one should construct a `function tree' much as an
expression tree\footnote{\url{https://en.wikipedia.org/wiki/Binary_expression_tree}}.

\autoref{fig:functree} illustrates the idea with a tree structure and the code is shown in
\autoref{lst:functree}
