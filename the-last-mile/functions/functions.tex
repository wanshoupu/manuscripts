...
Modern computing systems are hierarchical.
Myriad components, built on top of one another, are assembled together from visible buttons down
to silicon chips.
...
acrophobia in many.
A cute little function may invoke a shocking number of libraries,
...
Fortunately, no one needs to understand all the details at once, thanks to abstraction and modular
design.
Abstract thinking is one of the key traits of modern human beings and it is also the
bread-n-butter of software engineers in every aspect of their daily practice.
When we develop a piece of software, we seldom do it from ground up but utilizing existing,
pre-built modules-frameworks, libraries, APIs, \textit{etc}.
When one uses these pre-built modules, one is only concerned by how to use it by reading the
documentations much like recognizing a bottle of pills by its label and following the
directions of usage.
We never need to know the chemical ingredients or the molecular structure.

Abstraction and encapsulation in software starts with functions.
...
With this in mind, one should be in a better position to write his own functions, classes,
...
If your program works on the first run, you should be immediately suspicious.
