...
Well, a shuffled array of $N$ entities encodes information and thus contains `entropy'.
When sorted, the array is deprived of the encoded information and thus is void of entropy.
Aha, \texttt{a} sorting algorithm to a shuffled array is like a
...
number of `accessible micro-states' of the system.
This was first developed by Ludwig Boltzmann in the 19th century\cite{kittel1980thermal}.
Now if we take the $N$-particle system as the array of $N$ entities and all its permutations as
the `accessible microstates', then the formula suddenly makes sense\footnote{Time complexity
of an optimal sorting algorithm is proportional to the number of comparison operations which is
in turn proportional to its entropy
\begin{align*}
    S &= \log\left( N! \right) & \\
...
Good news is that the body of `best practices' has become established consensus.
...
Anyone who are striving for long-term investment of
improving their programming skills would find this book inspiring and beneficial, including
those who are preparing for technical interviews.
With that said, this book is intended for professionals who have already had a solid command of
...
I hope readers would find this book to be somewhat expected and somewhat unexpected.
