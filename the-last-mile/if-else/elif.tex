...
and ~\autoref{lst:staircaseCondReturn} are examples of staircase conditional.
\begin{clisting}
    \begin{minipage}[t]{.3\textwidth}
...
Execution of a staircase conditional is as follows.
Like stepping up a staircase, the control
successively evaluates the conditional expression and builds up the cases
progressively, one at a time.
The conditional expression of the leading conditional block is the first thing to check.
If it is evaluated to \code{true}, then the first block is executed and the rest is skipped.
...
the staircase conditional may do away with \code{else if}.
~\autoref{lst:if-stack-equals}, ~\autoref{lst:if-stack-lca},
...
If one needs to create intermediate variable in the middle, one should consider refactor
one's staircase.
...
Suppose that one needs the aid of a sound amplifier
in order to precisely distinguish the sounds of a snake and a bee, the current code would not
permit one to create a sound amplifier in the middle of the staircase.
There are more than one ways to deal with this.
One can refactor the code into a separate function so that one can break apart the tightly
coupled staircase conditional (~\autoref{lst:refactor_staircase_conditional}).
Or one can resort to a helper function to wrap away the amplification logic
...
\subsection{Cumulative effect of tests in a staircase conditional}

The effect of tests of a staircase conditional
is cumulative---a test in the staircase conditional accumulates the conditions of previous tests up
to and including its own, as
illustrated in ~\autoref{tab:cumulative-tests-staircase}.
