...
But in certain situations, the state of loop may not be updated at a steady pace for every
iteration which finds important applications in non-trivial algorithmic implementation.

From the perspective of choice of loop constructs,
\code{while}-loop is more accommodating for all kinds of stepping.
\code{for}-loop, on the other hand, has niches for both initialization and the afterthought
statement;
When using a \code{for}-loop, tuning the pace of iteration may be as simple as changing
the location of the state-updating statements, \textit{e.g.},
from an afterthought statement to a conditional block in the middle of the loop body.
