...
\subsection{Cumulative Tests in Staircase Conditional}\label{subsec:cumulativeStaircaseTests}
...
As can be seen, the program acquires one assertion through each test in a staircase
conditional, as if collecting a stamp in the course of a scavenger hunt.
This is the `cumulative effect' of tests in staircase conditional as
illustrated in ~\autoref{lst:cumulative-tests-staircase}.
...
On the one hand, the cumulative effect of staircase conditionals demands a strict ordering among
its conditional statements.
On the other, because of the cumulative effect, one has to commit more information in memory when
programming staircase conditionals and thus making it more challenging to
keep one's mind synchronized with one's code from time to time.
As explicit expansion of the staircase conditional,
~\autoref{lst:cumulative-tests-expanded-equivalent}, the side-by-side companion of
~\autoref{lst:cumulative-tests-staircase}, makes a straightforward worksheet for debugging and
code inspection.


\subsection{How to Arrange Conditionals in Staircase Construct}
\label{subsec:orderingStaircaseConditional}

Therefore arranging the conditionals in a staircase conditional construct is the key
from every point of view.
Apparently it will take considerable deliberation in order to produce correct, optimal and succinct
staircase conditional.
I strongly recommend that reader use due diligence to screen and arrange tests in a staircase
...
ruled out much like that of a binary search (see ~\autoref{fig:bsp}).
...
Similarly in ~\autoref{lst:if-stack-lca}, one faces the quadchotomy: both keys
...
tests within the staircase conditional.


Another clue comes from trichotomy or quadchotomy logic as we considered in the second step.
Using the example in ~\autoref{lst:if-stack-lca} again, I first test if both keys are
...
It is a total waste.
Generally speaking, it is not the best choice to scoop out a case in the middle.
Instead, we start with isolated, corner cases first and gradually working our way
into the thick of the puzzle.
...
%What are the critical aspects in arranging the conditional statements in a staircase fashion?
...
%Now how to order the conditional statements in a SC?
...
However, with a properly designed staircase conditional as we are going to show next,
a handful of lines are sufficient to handle the myriad possibilities.
Take a look at ~\autoref{lst:if-stack-lca}.
The cases are built from ground up like this:
\begin{enumerate}
    \item Test for nullity;
    \item Test for necessity of left branching;
    \item Test for necessity of right branching;
...
% TODO
% Provide comparisons of alternative tests
% for example,
% test for nullity
% test for straddling
% test for left
