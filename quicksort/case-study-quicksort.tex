...
But for more sophisticated implementations or to reduce partitioning overhead, more base
...
\begin{quote}
    ...
    Otherwise, invoke partitioning function to partition the array into two subarrays and an
...
\end{quote}
The implementation of the partitioning function is key to the quicksort algorithm.
The partitioning function does three things:
\begin{enumerate}
...
In all implementations of the partitioning function,
...
With this said, we are ready to discuss implementation of quicksort and its partitioning function.
Admittedly, implementing quicksort is quite tricky.
Among the quicksort partitioning schemes, best known are
...
In Hoare's scheme, two pointers from both ends of the array step toward each other;
whereas in Lomuto's, two pointers start off the same position
and step in the same direction on their own paces.
Let's start with a simpler version---Lomuto's partitioning scheme.


\subsection{Implementation of Lomuto's Partitioning Scheme}\label{subsec:impl-of-Lomuto-part-fun}
%TODO
...
\subsection{Implementation of Hoare's Partitioning Scheme}\label{subsec:impl-of-qsort-part-fun}

Hoare's scheme is the original partitioning scheme when the quicksort algorithm was
invented\cite{Hoare1961,Hoare1962,Sedgewick1978}.
However implementation of this partitioning function is very involved.
For a long time, few understand why this partitioning scheme works at all!
%TODO

As a spoiler, take a look at ~\autoref{apx:impl-varieties} where we listed a number of
implementations variations for the partitioning function.
...
changing others.
For example,
%the return value of the partitioning function is picked among .
...
After these changes are applied, our the partitioning function becomes
...
caption={Implementation of partitioning function for Quicksort using one pointer.}]
