...
In plain english, this is how quicksort works in ballpark:
\begin{quote}
    If the array contains fewer than $2$ elements, return as is.
    Otherwise, invoke partitioning function to partition the array into two subarrays.
...
\begin{enumerate}
    \item Pick a pivot element among the array and set it aside;
...
Pivot selection has prominent impact on the performance of quicksort algorithm.
Performance degradation can occur if the picked pivot suffer significant skewness.
The most na\"{i}ve way is to pick element at a fixed location.
For sneakily crafted input array, this pivot selection algorithm may cause severe skewness.
A simple but effective technique defying pivot skewness is randomly selection.
But random selection is usually pseudo-random and may sabotaged by malicious data.
Also random number generation may be time-consuming.
A more robust an fast way is to choose pivot element is using ``median-of-three'' technique:
from among $3$ sample elements
(\textit{e. g.}, the first, the middle, and the last elements), pick the median as the pivot.
``Median-of-three'' technique has been proven effective to prevent
runtime degradation\cite{sedgewick1977,sedgewick1978}.
As a caveat, one must ensure that the array contains at least $3$ elements to apply
``median-of-three'' technique.
But this is usually done regardless in conjunction with other performance tuning technique
so that it brings dual benefit.


\subsection{Implementation of Quicksort Partitioning Function}\label{subsec:impl-of-qsort-part-fun}
...
Hoare's scheme was invented along with the quicksort algorithm\cite{Hoare1961,Hoare1962,
Sedgewick1978}.
