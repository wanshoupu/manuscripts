...
What if one wants to refactor existing code concerning suboptimal conditionals?
...
This behavior is the default in \cpp STL `\code{std::map}' and
is made possible in \python if created by the following
\begin{lstlisting}
    lookup = collections.defaultdict(list)
\end{lstlisting}
Lastly, for better or for worse, you inherited the following code snippet from someone else:
\begin{clisting}
...
In general, refactor is a risky process.
The result may be better or worse---no happy ending is guaranteed.
Refactor process is, in a way, rather similar to quantum tunneling where electrons `proactively
dive' into energy-inhibited region, the so-called energy barrier, in search of lower energy minima.
So one must be prepared to see that before the code gets better, it may get worse in passing.
Also it remains unknown whether a better solution exist until one emerges on the other end
of the darkness.

Nevertheless, one has to overcome one's fear or laziness---`no pain no gain'.
To help cope with the hardship, I developed an `unnest' technique to convert a nested conditional
to a cascading one---for reference purposes, let us call it `conditional push-up'.
...
buried in the middle of the block and to bring it to the upper level, one risks introducing
duplicate code.
Just take note but do not be intimidated by that.
What compels us to move forward is the overwhelmingly strong
signal---the \code{if..else} blocks is overly unbalanced---and because of that there is no
turning back.

So let's what is the test for the nested conditional.
It is not hard to see that the test is \code{len(lookup[key]) == 1} because before the nested
conditional there is a \code{heappop} operation (and then if the list is empty, it is evicted out
of the dictionary thereafter).
So far so good.
...
Alright, please follow ~\fullref{sec:orderingstaircaseconditional} for the rest of this refactor
to arrange the conditionals into a cascade.
Shut up and code before turning the page.
...
At this moment the previously inaccessible refactor regarding the duplicate code `\code{heappush
(lookup[key + 1] = ..} was made easy
\lstinputlisting[firstline=28]{if-else/nested_conditional_2_cascading_conditional.py}
%TODO

Through this example, I am hopeful that the reader forms a `code-sense', an instinct, a habit
of optimization mindset.
First, know what lean code would look like
when coming to a similar situation, and then it would be more likely for one
to achieve such level of neatness.
For example, as to when should one trigger the `conditional push-up' process, it is not simply
``As soon as you spot one.''
Usually when I spot nested conditionals \emph{accompanied} by duplicate code smell, I would
consider refactor the code.


Of course, cascading conditional may not be the best solution still in occasions.
As you will learn later, the complexity of conditionals may also be designed away with
`object-oriented design'.
As to when to make a call on what to choose, I think aside from the degree of code reuse, it is
also a personal taste.
But for ~\autoref{lst:conditional-push-up-example}, it happens to be such a case that OOD is a
great choice.
