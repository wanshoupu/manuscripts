...
I will leave it an exercise for the reader to figure out why it was a bug.
As so, this code is admittedly very delicate and very hard to get right.
Third, \autoref{lst:toPrettyString}, function parameter `\code{indent}' is
...
But above all, the biggest problem to me is with the bloated, multitasking functions
\code{__dictToPrettyString__} and \code{__arrayToPrettyString__}.
Take a look at the function \code{__dictToPrettyString__}, it does at least two distinct
things---building a list of string representations for each item and assembling
...
But in our case, the formatting logic have to glue three things together---with indentation spaces
on top, element and field separator in between, and a newline at the bottom---
except the last element, for which NO field separator is used.
...
\end{figure}
How about we shift the boundary between lines so that both the indentation
and the newline character are on one side?
To that end, we need a perspective change.
See \autoref{fig:line-view} for an illustration.
We need renew our view of a `line' and make `cuts' such that lines taking the form
\[\mbox{line} = \mbox{content} + \mbox{newline}\]
are rotated to the form
\[\mbox{line} = \mbox{newline} + \mbox{content}.\]
Along with this new line-wise view, the meaning of
indenting a line also need to be redefined as ``Insert indentation
spaces between the \code{newline} and the rest of the line content''.
As you may se, with this counterintuitive `line cut',
it is made possible that
the styling strings---newline character and indentation string---be added on one side of
an element.
It is this minute change that unleashes the power of \code{str.join} function.
...
I will leave it as an exercise to the reader to refactor the ~\autoref{ArrayFormatting
.shifted_decoration} to get rid explicit code to achieve `all-but-last' separation
using the built-in function \code{str.join}.
