...
Mathematically speaking, a property is shared by all four---they are circular sequences with
certain rotational degrees of freedom.

If pictured as a Ferris wheel, distributed around are the statements of the loop
(see ~\autoref{fig:loop-rotation}).
Counting counterclockwise starting from the lowest node, the nodes in a
Ferris wheel may be ordered into linear sequence.
But due to the rotational degrees of freedom of the circular sequence,
Every statement can be the `first' (or `last') depending on which node is positioned lowest.
One thus may take advantage of this rotational degrees of freedom to
fine tune loop implementation by `rotating' the loop to match up and coalesce code in affinity
of the loop.
\begin{figure}[htb]
    \centering
...
Another helpful picture is to think of loops as spools and programs as threads---loops
are introduced to help organize threads.
Thus redundant code outside of the loop can coalesce into the loop body just like loose thread
can be wound up into a spool.
