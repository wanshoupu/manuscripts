...
To compile and run a simple ``Hello, World'' program written in C for example,
...
\begin{table}[H]
    ...
    \end{tabular}
\end{table}
Each of these components are by themselves a huge topic in computer science.
Remember this is just a simple program.
More complicated programs will likely involve I/O operations, network resources, database queries,
\textit{etc.}
...
Fortunately, you as a developer do not need to understand everything to work on a small part of
the system.
As the implementation details of many parts of the system have been wrapped into components and
modules and the directly related components and modules interact through a carefully designed
API's.
The implementation detail of each component or module has been hidden away.
This is called abstraction and encapsulation and it plays a key roll in software engineering.
In fact the ability to abstract in general is a unique trait of human being.
Without abstraction, human would not be able to think and reason very far.
Needless to say, the concept of abstraction has been applied everywhere in computer science.
Control flow abstraction resulted in the creation of high-level programming languages;
data abstraction enabled the creation of generic data structures;
object oriented programming is entirely based on object abstractions in hierarchy and composition.
...
As a chef need not know how to garden,
a developer need not know how the underlying components were implemented.
He or she may focus on how to put them together to accomplish the task at hand based on the API
specifications.
...
\begin{figure}[htb]
    \begin{center}
        ...
        \caption{Hierarchy of software system}
...
Designing clean, reusable functions is the foundation of software engineering,
especially object-oriented design.
In this chapter, we will get our hands dirty and show you how to write clean,
reusable functions step-by-step.
...
We will next ignore all the language-specific features but focus on what is
essential---the function name, the arguments, and the body.
...
Exactly what makes a function is an individual taste.
The general guideline is the the single-responsibility principle---a function should serve
exactly one purpose.
A rule of thumb judgement can be made based on the readability of a function---if it takes
substantial effort to figure out how the function body achieve what the function is supposed to
do, then consider refactor it into yet smaller functions.
