...
nested loops in situations where it could have been avoided.
Although permitted by most programming languages, nested loops almost always cause
hindrance to better readability and maintainability.
More often than not, they are mere nuisances when one tries to understand the semantics
or analyze the runtime complexity.
Occasionally, they may result in suboptimal implementation with degenerate runtime
complexity---\textit{e.g.}, turning a $O(N)$ algorithm into one of $O(N^2)$. %TODO example?
...
and think over it instead of jump into conviction.


%More often than not, situations that makes the use of nested loops so compelling
%and the situation seems to make nested loops inevitable.
...
we may dedicate this chapter to the study of study of simplifying nested loops.

The rule of thumb is ``Be cautious when using nested loops'.
...
Sometimes, situations that make nested loops appear so compelling or even inevitable are not so
if done with careful consideration.
where nested loops may be avoided is when implementing algorithms over linear data
