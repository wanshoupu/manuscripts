...
The implementation of the partition function is key to the success of quicksort algorithm.
The partition function does three things:
\begin{enumerate}
    \item pick a pivot element among the array $P$;
    \item partition the array (minus the pivot) into two subarrays $S$ and $G$ with
    $S \leq P$, $G \geq P$;
    \item arrange the subarrays and the pivot element such that $[S, P, G]$.
\end{enumerate}
...
A simple but effective technique to prevent pivot skewness is to randomly select the pivot element
from among the array and place it to the beginning of the array.
But random selection is pseudo-random and may itself may be time-consuming.
A more robust an fast way is to choose ``median-of-three'': from among $3$ sample elements
...
Correspondingly, one must ensure that the array contains at least $3$ elements to apply
``median-of-three'' technique.


Among the partitioning schemes for quicksort purposes, two well-known are
Hoare's scheme and Lomuto's scheme\cite{Hoare1961,Bentley:1986:PP}.
The main difference between them lies in the direction the array is traversed.
In Hoare's scheme, two pointers from both ends of the array press toward each other whereas in
Lomuto's, two pointers start off from one end and scan down one direction at differing pace.
...
In ~\autoref{apx:impl-varieties} we listed a variety of implementations that all work with
the same driving function \code{qsort}.
These listings are to be understood with appropriate pivot-selection measures to prevent severe
pivot skewness which in the worst cases will cause performance degradation to $O(N^2)$.
