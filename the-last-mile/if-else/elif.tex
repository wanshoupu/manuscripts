...
The control successively evaluates the conditional expression and builds up the cases, one at a
time---the
...
Passing through a staircase conditional, control progressively acquires a series of assertions ,
as if one acquires a series of stamps in a scavenger hunt.
This is illustrated in ~\autoref{tab:cumulative-tests-staircase}.


One property of staircase conditional is the strict order of conditional statements.
Designing the order of conditional statements takes a great deal of deliberation.
As to my own experience, I sometimes spend a long time permuting the conditional blocks aiming
for optimal performance and succinct code.


One of the most important reason for this is the cumulative effect of the sequence of tests
in a staircase conditional.


A test in the staircase conditional accumulates the conditions of previous tests up
to and including its own,
As illustrated in ~\autoref{tab:cumulative-tests-staircase}.


\begin{table}[tbh]
    \begin{center}
        \caption{\label{tab:cumulative-tests-staircase}
        Cumulative effect of tests in a case stack.
        Left: a staircase conditional construct.
        Right: sufficient and necessary conditions are expanded for each case.}
...
Keeping the consistency between one's mind and the code is even more challenging
when constructing staircase conditionals.
When debugging a staircase conditional, for example,
one needs to keep this cumulative effect in mind: that the test on the current line holds
negated in all subsequent blocks and that the SNC for the current line comes from all the
previously tested conditions.
All the assumptions about the the current line of code must have been tested by an earlier
...
The conditional statements are so tightly coupled that
...
For example creating an intermediate variable to be used in conditional expression in the middle
of a staircase conditional is not only very hard but also not encouraged.
Also, it is recommended that each conditional block in a staircase conditional
usually consists of \emph{no more than} two statements.
...
If one needs to create intermediate variable in the middle, one may have to refactor
one's staircase.
For example consider this problem ``Based on a given \code{sound}, construct an animal and add it
to the \code{collection}.''
...
Suppose that the sounds of snake and bee needs to be amplified in order to tell them apart,
one wants to create an amplifier instance.
But where should one create the amplifier instance?
Before the staircase conditional?
After all, the amplifier is not needed for any other sounds but that of snakes and bees.
Or maybe there is no snake or bee in the list then creating an amplifier would be a total waste.
Anyway one would want to create the amplifier as close to where it will used as possible.
Creating an amplifier at the beginning is not a good choice.
But on the other, there is no room for one to create amplifier instance
in the middle of the watertight staircase construct either.

There are more than one ways to overcome this dilemma.
One can refactor the staircase into a separate function where the
coupling among the conditional statements may be made less tight
(~\autoref{lst:refactor_staircase_conditional}).
Or one can resort to a helper function to wrap away the verbose amplification logic
(~\autoref{lst:modified_tight_staircase_conditional}).
...
For some real-world examples, see ~\autoref{lst:if-stack-equals}, ~\autoref{lst:if-stack-lca},
...
\begin{clisting}
    ...
    {if-else/equals.java}
\end{clisting}
As a real-world example to demonstrate the effectiveness of staircase conditional,
