...
\end{clisting}
How do we go about with refactoring nested conditionals?
We are going to present a technique, dubbed `conditional push-up', that is designed to convert
a nested conditional to a cascading one.
The technique is very simple.
First, identify conditions in the targeted nested conditionals.
Then, promote these conditionals to the upper level so that the conditions are merged.
The rest of the task is identical to what we have discussed above in order
to form a cascading conditional.


Taking the ~\autoref{lst:conditional-push-up-example} as an example, let us go over the process.
At first sight one may be intimidated by the fact that the nested if is buried in the
middle of the block.
Refactoring a nested conditional in the middle of a block is risky as it may introduce duplicate
code.
But just as a quantum tunneling is not guaranteed to find lower local minima,
no refactor attempt is guaranteed to succeed.
Particularly this one.
One will never leave the starting point with excessive expectation of a guaranteed,
luxurious happy ending.

But observing the an overly unbalanced \code{if..else} blocks, I perceive a strong signal
that compels me to refactor it.
%TODO
